...
void perform_elimination(int recv_id, int rank, int num_eq, double* proc_rows, double* proc_vals, int curr, double* recvd_row, int rows_per_proc, int numprocs, int * var_perm) {
...
int compute_pivot(int curr, int numprocs, int num_eq, double* proc_rows) {
	double mx = -1;
	int pivot;
	int row_id = curr / numprocs;
...
void perform_division(int rank, int curr, double* proc_rows, int pivot, int numprocs, int num_eq, int rows_per_proc, double * proc_vals) {
...
int main( int argc, char **argv ) {
	...
	*/
	...
	/* for flattening */
	int divs[numprocs]; 	// Number of coefficients per proc
	int displs[numprocs]; 	// Begining index of coefficients of each process
	...
	int rpp = num_eq / numprocs; // temporary variables
	 
	int np0 = numprocs - (num_eq % numprocs); 	// Number of processes with rows = rpp (np0),
	int np1 = numprocs - np0; 					// number of processes with rows = rpp + 1(np1)
	...
	if(rank == 0) {    
		...
		for(int i = 0; i < num_eq; i++) {
			...
			int assigned_proc = i % numprocs; // process assigned for the current row to be read
	...
	int next_proc = (rank + 1) % numprocs;

	/* The process which sends the data to the current process */
	int prev_proc = (rank - 1 + numprocs) % numprocs;
	
	int curr = rank; 					// The row which is being processes currently
	...
	*/
	for(int cnt = 0; cnt < rows_per_proc; cnt++) {
		...
		for(int i = prev_curr + 1; i < curr; i++) {
			...
			/* 
				The recieved row has traversed (curr - i) number of times, so it it has traversed numprocs or more times, 
				it has been delivered to all processes 
				TODO: check this
			*/
			if(curr - i < numprocs - 1) {
			...
			perform_elimination(i, rank, num_eq, proc_rows, proc_vals, curr, recvd_row, rows_per_proc, numprocs, var_perm);
		}
		piv = compute_pivot(curr, numprocs, num_eq, proc_rows); 

		// int tmp = var_perm[piv];
		// var_perm[piv] = var_perm[curr];
		// var_perm[curr] = tmp;

		perform_division(rank, curr, proc_rows, piv, numprocs, num_eq, rows_per_proc, proc_vals);
		...
		perform_elimination(prev_curr, rank, num_eq, proc_rows, proc_vals, curr, send_buf, rows_per_proc, numprocs, var_perm);
	}
	...
	*/
	...
	int lst = rank + (rows_per_proc - 1) * numprocs; // actual rank of the last row of the current process
	int prev_lst = num_eq;
	double x_val;
	int count = 1;
	...
	if(rank == 0) {
		...
		for(int i = 0; i < numprocs; i++) {
			for(int j = i; j < num_eq; j += numprocs) {
				solution[j] = res[k++];
			}
		}
